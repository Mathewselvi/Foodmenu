...
const importData = async () => {
    try {
        ...
        for (let line of lines) {
            line = line.trim();
            if (!line || line === '⸻' || line.includes('(All items exactly included') || line === 'add these products to the project with prices and every thing admin should need to update price when ever he needs do it') {
                continue;
            }

            // If it's a category header, update currentCategory
            if (!line.startsWith('*') && !line.includes('–')) {
                // remove emoji if present by stripping non-alphanumeric chars at the VERY START
                currentCategory = line.replace(/^[^\w]+/, '').trim();
                continue;
            }

            // It's a product line like: * Black Tea – 10.00 or * Chicken Masala – 150 / 260
            if (line.startsWith('*')) {
                const parts = line.replace('*', '').split('–');
                if (parts.length < 2) continue; // safety check

                let name = parts[0].trim();
                let pricesPart = parts[1].trim();

                // If Shawaya, append category name so "Half" becomes "Shawaya Half"
                if (name === 'Half' || name === 'Full') {
                    name = currentCategory + " " + name;
                }

                // Split prices by /
                const priceValues = pricesPart.split('/').map(p => parseFloat(p.trim()));
        ...
        console.log("Successfully loaded " + productsToInsert.length + " new items into the cloud database.");
        process.exit();
    } catch (error) {
        console.error(error);
        process.exit(1);
